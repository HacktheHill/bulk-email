...
export function validateRenderedMessage(input: {
	...
	const hasPlaceholderMarkup = input.html.includes("{{") || input.text.includes("{{");
	if (hasPlaceholderMarkup && (UNRESOLVED_PLACEHOLDER_REGEX.test(input.html) || UNRESOLVED_PLACEHOLDER_REGEX.test(input.text))) {
