...
const UNRESOLVED_PLACEHOLDER_REGEX = /\{\{\s*[A-Za-z][A-Za-z0-9_.]*\s*\}\}/;
...
export function validateRenderedMessage(input: {
	...
	// Fast path: Only run the expensive regex if the placeholder prefix is present in the large rendered strings
	if (input.html.includes("{{") && UNRESOLVED_PLACEHOLDER_REGEX.test(input.html)) {
		throw new Error("Template contains unresolved placeholders");
	}
	if (input.text.includes("{{") && UNRESOLVED_PLACEHOLDER_REGEX.test(input.text)) {
		throw new Error("Template contains unresolved placeholders");
	}
