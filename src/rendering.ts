...
export function validateRenderedMessage(input: {
	...
	const placeholderRegex = /\{\{\s*[A-Za-z][A-Za-z0-9_.]*\s*\}\}/;
	// ⚡ Bolt: Fast-path check using `.includes` before running the expensive regex, as placeholders are usually already resolved
	if (
		(input.html.includes("{{") && placeholderRegex.test(input.html)) ||
		(input.text.includes("{{") && placeholderRegex.test(input.text))
	) {
