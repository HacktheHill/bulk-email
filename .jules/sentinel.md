...
## 2024-08-25 - Prototype Lookup DoS
**Vulnerability:** Unsubscribe key ID derived from user input used directly for object property lookup on a plain object. User can pass `constructor` or `__proto__` to get a function/object back, which crashes `createHmac` causing DoS.
**Learning:** Plain Javascript objects `{}` inherit properties from `Object.prototype`. Using user-provided keys for direct lookup allows accessing these inherited properties.
**Prevention:** Use `Object.create(null)` or `Map` when using JavaScript objects as dictionaries for user-provided data or JSON parsing.
